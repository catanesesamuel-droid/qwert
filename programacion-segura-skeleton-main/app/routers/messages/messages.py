...
def create_message(payload: MessageCreate, user=Depends(get_current_user), session: Session = Depends(get_session)):
...
def list_my_messages(user=Depends(get_current_user), session: Session = Depends(get_session)):
    db_user = session.exec(select(User).where(User.username == user["username"])).first()
    msgs = session.exec(select(Message).where(Message.owner_id == db_user.id)).all()
    return msgs

@router.delete("/{message_id}", status_code=204)
def delete_message(message_id: int, user=Depends(get_current_user), session: Session = Depends(get_session)):
